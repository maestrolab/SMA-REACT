...
def analyze_mts(file, end, interval, glitch_check, temp_title, disp_title, shape, unit_out, area, movavg_conditions, datapoints_dict):
    # EXTRACTING MTS DATA
    mts = reader(file)
    mts.extract()
    ...
    start_time = input("Enter the start time for the MTS (MM/DD/YYYY HH:MM:SS): ")
    ...
    List_of_loads, mov_avg_set, unit, figure = movavg_filter(mts_data, temp_title, disp_title, movavg_conditions, datapoints_dict)

    # INTERPOLATING THE MOVING AVERAGES
    for column in mts_data.columns:
        if "Moving Average" in column:
            mts_data[column] = mts_data[column].interpolate(method="linear", limit_direction="backward")
