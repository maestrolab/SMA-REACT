...
def movavg_filter(df, temp_title, disp_title, mov_avg_set, datapoints, fluke_check=False, fluke_temp_title=""):
    ###############################################################################################################
    ###############################################################################################################
    ...
    if 'all' in mov_avg_set:
        ...
        if 'temperature' in mov_avg_set:
            ...
            if fluke_check:
                fluke_temp_MovAvg_points = []
        if 'displacement' in mov_avg_set:
            ext_MovAvg_points = []
    ...
    if len(mov_avg_set) != 0:
        ...
        while satisfaction == 'no':
            ...
            if mov_avg_set == {'all'}:
                ...
                ax1.plot(mts_temp, extension)
                ax1.plot(filtered_temp, filtered_extension)
                ax1.legend(["Unfiltered measurements, Filtered measurements"])
                ax1.set_title("Filtered Extension and Filtered MTS Temp")
                ax2 = figure.add_subplot(232)
                ax2.plot(mts_temp, extension)
                ax2.plot(mts_temp, filtered_extension)
                ax2.legend(["Unfiltered measurements, Filtered measurements"])
                ax2.set_title("Filtered Extension and Unfiltered MTS Temp")
                ax3 = figure.add_subplot(233)
                ax3.plot(mts_temp, extension)
                ax3.plot(filtered_temp, extension)
                ax3.legend(["Unfiltered measurements, Filtered measurements"])
                ax3.set_title("Unfiltered Extension and Filtered MTS Temp")
                ax4 = figure.add_subplot(234)
                ax4.plot(mts_temp, load)
                ax4.plot(filtered_temp, filtered_load)
                ax4.legend(["Unfiltered measurements, Filtered measurements"])
                ax4.set_title("Filtered Load and Filtered MTS Temp")
                ax5 = figure.add_subplot(235)
                ax5.plot(mts_temp, load)
                ax5.plot(mts_temp, filtered_load)
                ax5.legend(["Unfiltered measurements, Filtered measurements"])
                ax5.set_title("Filtered Load and Unfiltered MTS Temp")
                ax6 = figure.add_subplot(236)
                ax6.plot(mts_temp, load)
                ax6.plot(filtered_temp, load)
                ax6.legend(["Unfiltered measurements, Filtered measurements"])
                ax6.set_title("Unfiltered Load and Filtered MTS Temp")
                ...
                #plt.savefig("movavg_plot.png", dpi=120)
                if fluke_check:
                    fig, axs = plt.subplots(2,3)
    ...
    return List_loads, mov_avg_set, unit, figure
